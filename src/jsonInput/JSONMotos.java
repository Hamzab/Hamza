   ...
   public JSONMotos(){
    ...
    public  int getAnnee(int indice){
        ...
        try{
            annee=getMotos().getJSONObject(indice).getInt("annee");
             ...
             messagesErreurs.add("Erreur fromat json ,le moto ne contient pas l'annee");
        }
        return annee;
    }
    ...
    public  String getMarque(int indice){
        ...
        try{
            ...
            System.out.println(marque);
             messagesErreurs.add("Erreur fromat json ,le moto ne contient pas la marque");
        }
        return marque;
    }
    ...
    public  String getModele(int indice){
        ...
        try{
          modele=getMotos().getJSONObject(indice).getString("modele");  
           ...
           messagesErreurs.add("Erreur fromat json ,le moto ne contient pas le modele"); 
        }
        return modele;
    }
    ...
    public  double getValeurDesOptions(int indice)  {
        ...
        try{
           valeur= getMotos().getJSONObject(indice).getDouble("valeur_des_options");
             ...
             messagesErreurs.add("Erreur fromat json ,le moto ne contient pas la valeur des options");
        }
        return valeur;
    }
    ...
    public  String getBuriange(int indice) {
        ...
        try{
           ...
           messagesErreurs.add("Erreur fromat json ,le moto ne contient pas le burinage"); 
        }
        return burinage;
    }
    ...
    public  boolean getGarageInterieur(int indice) {
        ...
        try{
           garageInt= getMotos().getJSONObject(indice).getBoolean("garage_interieur");
            ...
            messagesErreurs.add("Erreur fromat json ,le moto ne contient pas s'il y a un garage interieur");  
        }
        return garageInt;
    }
    ...
    public  boolean getSystemeAlarme(int indice) {
        ...
        try{
           ...
           messagesErreurs.add("Erreur fromat json ,le moto ne contient pas s'il y a un system d'alarme");   
        }
        return systemAlarm;
    }

    public int getValeurCC(int indice) {
        int valeur=0;
                try{
           valeur=getMotos().getJSONObject(indice).getInt("CC"); 
        }catch(Exception e){
           messagesErreurs.add("Erreur fromat json ,le moto ne contient pas valeur cc");   
        }
        return valeur;
    }
